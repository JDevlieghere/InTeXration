...
class Server:
    ...
    def _log(self, owner, repository, name):
        try:
            ...
            try:
                ...
                abort(404, "The requested document does not exist.")
        return template(os.path.join(self.config.file_path('templates'), 'log'),
    ...
    def _index(self):
        return template(os.path.join(self.config.file_path('templates'), 'index'), root=self.config.server_root())
